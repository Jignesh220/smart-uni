...
export default function Page({
  ...
  params: { semester: string; mainsubject: string; degree: string,category: string };
}) {
      ...
      <div className="mt-16 min-w-full px-32">
        <div className="min-w-full flex flex-row flex-wrap justify-center gap-3">
          {MainSubjectData.map((item) => (
            <Link href={`/category/${params.category}/${params.degree}/${params.semester}/${params.mainsubject}/${item.url}`} key={item.id}>
              <motion.div
                initial={{
                  scale: 0,
                  opacity: 0,
